...
const I18N = {
  fr: {
    ...
    heroSub:
      "Sélectionne tes modules M1 Info S2, choisis ton groupe TD et télécharge ton calendrier .ics.",
    loadingData: "Chargement des données...",
    errorLoad: "Données indisponibles.",
    errorNoData:
      "Les données CELCAT ne sont pas encore disponibles. Le fichier sera généré automatiquement par le workflow GitHub Actions.",
    retry: "Réessayer",
    ...
    configDesc:
      "Télécharge ce fichier calendar-config.json et mets-le dans ton propre dépôt GitHub pour obtenir un calendrier auto-actualisé.",
    configSteps: [
    ...
    updateDesc:
      "Envoie un fichier .ics généré précédemment. On détectera tes modules et téléchargera une version actualisée.",
  ...
  en: {
    ...
    heroSub:
      "Select your M1 Info S2 modules, pick your TD group, and download your .ics calendar.",
    loadingData: "Loading calendar data...",
    errorLoad: "Data unavailable.",
    errorNoData:
      "CELCAT data is not yet available. The file will be auto-generated by the GitHub Actions workflow.",
    retry: "Retry",
    ...
    configDesc:
      "Download this calendar-config.json and put it in your own GitHub repository to get an auto-updating calendar.",
    configSteps: [
    ...
    updateDesc:
      "Upload a previously generated .ics file. We'll detect your modules and download a refreshed version.",
...
const ROOM_RE =
  /<br\s*\/?>\s*([A-Z0-9][A-Za-z0-9 ]*\s+-\s+[A-Z][A-Za-z0-9 /().,'\u2013&;#\-]+\[.+?\])/;
const EXAM_KEYWORDS = [
  "examen",
  "partiel",
  "contrôle",
...
function extractModuleCodes(text) {
  ...
  while ((m = MODULE_CODE_RE.exec(text)) !== null)
...
function filterEvents(rawEvents, modules, includeExams) {
  ...
  const enrolledCodes = new Set(
    Object.keys(modules).map((c) => c.toUpperCase())
  );
  const kept = [];
  const seen = new Set();
  ...
  for (const raw of rawEvents) {
    ...
    if (!isExam && (category.startsWith("TD") || category.startsWith("TP"))) {
      ...
      if (groupNums.length) {
        ...
        for (const code of matchingCodes) {
          ...
          if (expected != null && groupNums.includes(expected)) {
            match = true;
            break;
          }
        }
    ...
    if (matchingCodes.length) {
      ...
      const modName =
        (modules[primaryCode] && modules[primaryCode].name) || primaryCode;
    ...
    const color =
      raw.backgroundColor ||
      (isExam ? COLORS.EXAM : COLORS[category] || COLORS.DEFAULT);
